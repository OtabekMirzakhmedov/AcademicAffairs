...
export class AuditController {
  ...
  @Get()
  @ApiOperation({ summary: 'Get audit logs', description: 'Paginated audit log with optional filters (admin only)' })
  ...
  @Get('entity/:type/:id')
  @ApiOperation({ summary: 'Get entity audit history', description: 'Full history of one entity (admin only)' })
  @ApiParam({ name: 'type', description: 'Entity type e.g. TeachingActivity, User' })
  ...
  async findByEntity(
    ...
    return { success: true, data: await this.auditService.findByEntity(type, id) };
  }

  @Get('actor/:userId')
  @ApiOperation({ summary: 'Get actor audit history', description: 'Everything an actor did (admin only)' })
